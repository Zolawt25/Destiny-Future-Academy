  ...
  return ( 
    <div className='sticky top-0 flex justify-center bg-[#F96106]'>
      <div className={`flex items-center justify-between py-2 md:px-20 px-10 shadow-md relative ${responsiveNav&&"overflow-hidden"} max-w-[1400px] w-full`}>
        ...
        <div className={`absolute md:relative top-0 h-screen md:h-auto z-50 bg-[#F96106] md:bg-transparent ${responsiveNav ? "right-[-3000px] md:right-0" :"right-0"} flex flex-col md:flex-row w-[270px] md:w-auto items-center justify-center text-lg md:text-[15px] duration-300`}>
          ...
          <button className='absolute top-7 left-4 text-3xl font-semibold md:hidden' onClick={()=>setResponsiveNav(true)}>x</button>
        </div>
        <button className=' rotate-90 font-bold text-lg md:hidden' onClick={()=>setResponsiveNav(false)}>|||</button>
      </div>
    </div>
  )
}
